...
export default function vaultRoutes(pool: Pool) {
    const router = Router();
    ...
    router.get('/salt', async (req: AuthRequest, res: Response) => {
        ...
        try {
            const result = await pool.query(
                'SELECT master_salt FROM users WHERE username = $1',
                [user]
            );
    ...
    router.post('/verify', async (req: AuthRequest, res: Response) => {
        ...
        try {
            ...
            if (isValid) {
                const token = jwt.sign({ user_id: row.id }, process.env.JWT_SECRET as string, {
                    expiresIn: '1h',
                });
    ...
    router.get('/vault/get', validateToken, async (req: AuthRequest, res: Response) => {
    ...
    router.post('/vault/store', validateToken, async (req: AuthRequest, res: Response) => {
        ...
        try {
            await pool.query(
                ...
                [user_id, JSON.stringify(service), JSON.stringify(login), JSON.stringify(password), JSON.stringify(notes || {})]
            );
    ...
    router.delete('/vault/delete/:id', validateToken, async (req: AuthRequest, res: Response) => {
        const user_id = req.user?.user_id;
